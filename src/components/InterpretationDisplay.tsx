...
const InterpretationDisplay = ({
  ...
  const visibleInterpretations = interpretations.filter(interp => {
    switch (interp.type) {
      case 'islamic': return userPreferences.showIslamic;
      case 'spiritual': return userPreferences.showSpiritual;
      case 'psychological': return userPreferences.showPsychological;
      default: return true;
    }
  });
  ...
  return (
    <div className="max-w-md mx-auto p-4 space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm text-dream-gray">Your Dream</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-dream-navy">{dreamText}</p>
        </CardContent>
      </Card>
      ...
      {visibleInterpretations.length > 0 && (
        <>
          ...
          {visibleInterpretations.map((interp) => (
            activeTab === interp.type && (
              <Card key={interp.type} className="border-l-4" style={{ borderLeftColor: interp.color.replace('bg-', '#') }}>
                <CardHeader>
